...
Make sure that random characters are generated per user's choice (if they choose yes on uppercase letters, lowercase letters, numbers, special characters).
After user selects their prompts the password is displayed on the box on webpage.
Connect the event listener on the generate password button so the process can repeat each time the user clicks on it.
